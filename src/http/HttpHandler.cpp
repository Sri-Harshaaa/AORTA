...
std::size_t title_key_position =
request.body.find("\"title\"");

if(title_key_position != std::string::npos) {
std::size_t title_colon_position =
request.body.find(
':',
title_key_position
);

if(title_colon_position != std::string::npos) {
std::size_t quote_start =
request.body.find(
'\"',
title_colon_position + 1
);
...
std::size_t completed_key_position =
request.body.find("\"completed\"");

if(completed_key_position != std::string::npos) {
std::size_t completed_colon_position =
request.body.find(
':',
completed_key_position
);

if(completed_colon_position != std::string::npos) {
std::size_t value_start =
completed_colon_position + 1;

while(
value_start < request.body.size()
&&
std::isspace(
static_cast<unsigned char>(request.body[value_start])
)
) {
++value_start;
}

if(request.body.compare(value_start, 4, "true") == 0) {
completed_value = "true";
} else if(request.body.compare(value_start, 5, "false") == 0) {
